...
# ---------------------------------------------------------------------------
# strict numeric allow-list
# ---------------------------------------------------------------------------

def test_identifier_bearing_fields_are_ignored_even_when_explicitly_configured():
    out = format_runtime_footer(
        fields=("model", "context_pct", "cwd", "session_id"),
        counters=RuntimeCounters(1, 2, 3, 4, 5, 6, 7),
    )
...
def test_shipped_config_is_default_off_and_uses_only_numeric_fields():
...
def test_read_local_session_counters_fails_soft_on_older_schema(tmp_path):
    db_path = tmp_path / "state.db"
    with sqlite3.connect(db_path) as conn:
...
def test_read_local_session_counters_fails_soft_when_database_is_locked(tmp_path):
    db_path = tmp_path / "state.db"
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            """CREATE TABLE sessions (
...
def test_footer_transport_is_token_free_and_history_invariant(tmp_path):
    ...
    def run(enabled):
        ...
        db.update_token_counts(
            ...
            cache_read_tokens=provider.tokens["cache_read"],
            cache_write_tokens=provider.tokens["cache_write"],
            reasoning_tokens=provider.tokens["reasoning"],
    ...
    assert off[0].tokens == on[0].tokens == {
        "input": 111,
        "output": 22,
        "cache_read": 33,
        "cache_write": 4,
    ...
    assert on[3] == (
        "unchanged answer\n\n"
        "api 1 · in 111 · out 22 · cache 33 · cache+ 4 · reason 5"
    )
