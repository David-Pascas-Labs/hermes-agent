...
Supported numeric fields are `api_calls`, `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_write_tokens`, `reasoning_tokens`, and `tool_calls`. Legacy `model`, `context_pct`, and `cwd` fields remain accepted only when explicitly configured; avoid `cwd` on shared messaging surfaces because paths can contain personal or client names. Per-platform overrides remain available under `display.platforms.<platform>.runtime_footer`.
